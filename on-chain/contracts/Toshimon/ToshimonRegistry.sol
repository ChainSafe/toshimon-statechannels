 ...
 */
 ...
 contract ToshimonRegistry is AccessControl {
    ...
    mapping(uint256 => address) public moves;
    ...
    mapping(uint256 => address) public statusConditions;
     ...
     */
    function addMove(uint256 index, address contractAddress) public {
     ...
     */
    // function callMove(uint256 index, TM.GameState memory state, uint8 mover, bytes32 randomSeed) public pure returns (TM.GameState memory) {
     ...
     */
    function addItem(uint256 index, address contractAddress) public {
        require(hasRole(MANAGER_ROLE, msg.sender), "Caller is not a manager of the registry");
     ...
     *     - The implementation of applyMove throws an error
     */
    // function callItem(uint256 index, TM.GameState memory state, uint8 mover, uint8 monster, bytes32 randomSeed) public pure returns (TM.GameState memory) {
